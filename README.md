...
### Питание
1. **220В AC** → вход AC-DC преобразователя (220В → 5В)
2. **5В** → вход линейного стабилизатора (5В → 3.3В)
3. **3.3В** → VCC микроконтроллера (CB3S(для esp8266 аналогично))
4. **GND** → GND микроконтроллера
...
- Использовать только изолированный AC-DC преобразователь
